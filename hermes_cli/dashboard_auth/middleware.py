...
def _auto_sso_response(request: Request) -> Response | None:
    ...
    if getattr(provider, "supports_password", False):
        # Password-only providers (basic) have no OAuth redirect flow —
        # start_login raises NotImplementedError, so bouncing to /auth/login
        # would 500. Fall through to the /login form instead.
        return None
    prefix = prefix_from_request(request)
    next_param = _safe_next_target(request)
